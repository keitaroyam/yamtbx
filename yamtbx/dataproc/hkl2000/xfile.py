...
class DenzoXfile(object):
    ...
    def read_file(self, filein, onlyfull=False, onlypartial=False, read_I_sum=False):
        ...
        for i in range(3):
            vals = list(map(float, ifs.readline().strip().split()))
        ...
        self.osc_start, self.osc_end, self.distance, self.wavelength, self.rotz,self.roty,self.rotx, self.mosaicity = list(map(float, tmp))
        ...
        for l in ifs:
            if l.startswith(" 999 "):
                boxsize = list(map(int, l.strip().split()))[1:]
                break
            
            #sp = l.strip().split()
            sp = l[:4], l[4:8], l[8:12], l[13], l[14:22], l[22:30], l[30:37], l[37:43], l[43:49], l[49:56], l[56:63], l[63:69], l[69:77]
            h, k, l, isfull = list(map(int, sp[:4]))
            Ipf, Ips, chisq, sigma, cos, calx, caly, lpo, strength = list(map(float, sp[4:]))
        ...
        for l in ifs:
            if l.startswith(" "):
                self.intbox.append(list(map(int, l.strip())))
            elif l.startswith("unit cell"):
                self.unit_cell = list(map(float, l[9:].strip().split()))
